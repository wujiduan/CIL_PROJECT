...
def CRF(eval_img, eval_mask, use_label=False):
    ...
    sigma_smooth = 8.575103751642672
    sigma_color = 2.0738539891571977
    sigma_color_pos = 20
    ...
    w_smooth = 3.7946478055761963
    w_appearance = 1.8458537690881878

    iter_num = 5
    # add color-independent term, features are the locations
    d.addPairwiseGaussian(sxy=(sigma_smooth, sigma_smooth),
                          compat=compat_smooth.astype(np.float32),
                          kernel=dcrf.DIAG_KERNEL,
                          normalization=dcrf.NORMALIZE_SYMMETRIC)
    # add color-dependent term, features are (x, y, r, g, b)
    d.addPairwiseBilateral(sxy=(sigma_color_pos, sigma_color_pos),
    ...
    return MAP.reshape((height, width))
...
train_path = '/cluster/home/xindzuo/CIL_PROJECT/training'
val_path = '/cluster/home/xindzuo/CIL_PROJECT/validation'
...
folder_name = '/cluster/home/xindzuo/CIL_PROJECT'
# folder_name = ''
num_val = VAL_SIZE * 2
import gc
...
def train(train_dataloader, eval_dataloader, model, loss_fn, metric_fns,
          optimizer, n_epochs):
    # training loop
    logdir = '/cluster/home/xindzuo/CIL_PROJECT/tensorboard/net'
...
train_dataset = ImageDataset('/cluster/home/xindzuo/CIL_PROJECT/training',
                             device,
                             use_patches=False,
                             resize_to=(384, 384))
val_dataset = ImageDataset('/cluster/home/xindzuo/CIL_PROJECT/validation',
                           device,
                           use_patches=False,
                           resize_to=(384, 384))
...
torch.save(
    model.state_dict(), folder_name + '/model/model_xindi' + str(n_epochs) +
    '_val' + str(num_val) + '.pt')

test_path = '/cluster/home/xindzuo/CIL_PROJECT/test_images'
...
test_pred_crf = [
    CRF(ori_test_images[i], test_pred[i])
    for i in range((len(ori_test_images)))
]
test_pred_crf = np.stack(test_pred_crf)
test_pred_crf = test_pred_crf.reshape(
    (-1, size[0] // PATCH_SIZE, PATCH_SIZE, size[0] // PATCH_SIZE, PATCH_SIZE))
test_pred_crf = np.moveaxis(test_pred_crf, 2, 3)
test_pred_crf = np.round(np.mean(test_pred_crf, (-1, -2)) > CUTOFF)
create_submission(test_pred_crf,
                  test_filenames,
                  submission_filename=folder_name +
                  '/predict/unet_submission_e' + str(n_epochs) + '_val' +
                  str(num_val) + '_best_crf.csv')
